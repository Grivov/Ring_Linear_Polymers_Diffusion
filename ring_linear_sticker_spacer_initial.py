...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 11 13:13:29 2024
...
def index_assignment(length):
    """
    Assign indecies 1 for first sticker, 2 for second sticker or 3 for spacer
    
    Parameters:
    length (int): The length of the walk.

    Returns:
    tuple: Array of indecies.
    """
    iteration_number = int(length / 4)
    
    stickers_1 = np.tile([3,1], iteration_number)
    
    stickers_2 = np.tile([3,2], iteration_number)
    
    return np.append(stickers_1, stickers_2)


def generate_coords_and_idx(length, diameter, positions_y, positions_z):
    """
    ...
    chain_number = 0
    coords_all = {'x': [], 'y': [], 'z': [], 'chain_number': [], 'index': []}
...
def create_file(name, mode, length, diameter, x_box_dim, positions_y, positions_z, linear):
    ...
    """
    coords_all = generate_coords_and_idx(length, diameter, positions_y, positions_z)
    ...
    with open(name, mode) as f:
        ...
        for i in range(num_atoms):
            ...
            index = coords_all['index'][i]
            f.write(f'{i + 1} {chain_number} {index} {round(x, 4)} {round(y, 4)} {round(z, 4)}\n')
